...
状态栏本体只显示主 Codex 额度，使用单行紧凑布局：

```text
5h  [连续额度条]  69%  03:05    Week  [连续额度条]  95%  4/29
```

- 额度条表示剩余额度，不是已用额度。
- 额度充足时为绿色，降低后逐步转为黄/橙/红。
- `5h` 重置显示本地 `HH:mm`，例如 `13:19`。
- `Week` 重置显示 `M/d`，例如 `6/18`。
- 状态栏本体不显示 `Codex`、`剩余` 或 `重置` 这类冗余文字。
- 点击菜单会显示更完整的主额度、附加模型额度和 awaiting approval 操作。
